...
use, intrinsic :: iso_c_binding, only : c_double, c_float, c_ptr, c_int, c_size_t, c_sizeof, c_f_pointer, c_associated
...
if (status /= 0) then
    write (error_unit, '(A, I0, A, I0, A)') 'can not allocate ', n, ' by ', n, ' matrix'
    stop 2
end if
...
if (.not. c_associated(b_dev_ptr)) then
    write (error_unit, '(A, I0, A, I0, A)') 'can not allocate ', n, '*', n, ' device array'
    stop 2
end if
